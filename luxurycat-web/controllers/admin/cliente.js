    ...
    if (DATA.status) {
        if (action === 'searchRows' && DATA.dataset.length === 0) {
            TABLE_BODY.innerHTML += `
                <tr>
                <td class="col-1"></td>
                <td class="col-2"><b>${DATA.error}</b></td>
                <td class="col-2"></td>
                <td class="col-2"></td>
                <td class="col-3"></td>
                <td class="col-2"></td>
            ...
            DATA.dataset.forEach(row => {
                ...
                TABLE_BODY.innerHTML += `
                    <tr>
                    <td class="col-1">${row.usuario_id}</td>
                    <td class="col-2">${row.usuario_nombre}</td>
                    <td class="col-2">${row.usuario_apellido}</td>
                    <td class="col-2">${row.usuario_usuario}</td>
                    <td class="col-3">${row.usuario_correo}</td>
                    <td class="col-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: ${color_estado};" onclick="openState(${row.usuario_id})">
...
const openState = async (id) => {
    ...
    try {
        if (RESPONSE) {
            ...
            console.log(id);
            const DATA = await fetchData(CLIENTE_API, 'changeStatus', FORM);
...
const openReport = () => {
    ...
    const PATH = new URL(`${SERVER_URL}reports/admin/reporte_general_clientes.php`);
