...
if ($dataproductos = $productos->readAll()) {
    ...
    foreach ($dataproductos as $rowproductos) {
        // Define el estado del producto basado en el valor de 'producto_estado'
        ($rowproductos['producto_estado']) ? $estado = 'Activo' : $estado = 'Inactivo';
        ...
        $pdf->cell(100, 15, $pdf->encodeString($rowproductos['producto_nombre']), 1, 0, 'C'); // Celda de nombre
        $pdf->cell(25, 15, $pdf->encodeString($rowproductos['producto_cantidad']), 1, 0, 'C'); // Celda de cantidad
        $pdf->cell(30, 15, $pdf->encodeString($rowproductos['producto_precio']), 1, 0, 'C'); // Celda de precio
        $pdf->cell(30, 15, $estado, 1, 1, 'C'); // Celda de estado
    }
} else {
